...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        return max([self.getQValue(state, action) for action in legalActions]) if len(legalActions) != 0 else 0.0
    ...
    def update(self, state, action, nextState, reward):
        ...
        sample = reward + (self.discount * max([self.getQValue(nextState, nextAction) for nextAction in nextLegal])) if len(nextLegal) != 0 else reward
